...
const ARTIST_PAGE_BLOCK = [
	...
	"    };",
	"",
	"    // in-note trigger for the Wikipedia bio — same Templater command the",
	"    // hotkey fires, so it also works on mobile where there are no hotkeys",
	'    const BIO_COMMAND = "templater-obsidian:Templates/Scripts/Enrich Artist.md";',
	"    const canFetchBio = !!dc.app.commands?.findCommand?.(BIO_COMMAND);",
	"",
	...
	'                    <a onClick={() => dc.app.commands.executeCommandById(BIO_COMMAND)} style={{ cursor: "pointer" }}>⟳ Fetch Wikipedia bio</a>',
	"                </div>",
	"            )}",
